    ...
    <div class="alpenia-dashboard-shell">
        ...
        return '<div class="alpenia-message">' . esc_html(alpenia_travel_t('Kein Zugriff.')) . '</div>';
            ...
            <?php else : ?>
                ...
                <section class="operations-overview" aria-label="<?php echo esc_attr(alpenia_travel_t('Operations-Zentrale')); ?>">
                    <div class="operations-overview__header">
                        <div>
                            <span class="overview-card__eyebrow"><?php echo esc_html(alpenia_travel_t('Operations')); ?></span>
                            <h2><?php echo esc_html(alpenia_travel_t('Operations-Zentrale')); ?></h2>
                            <p><?php echo esc_html(alpenia_travel_t('Backoffice-Aufgaben, Dokumenten-Engpässe und kritische Reisen klar priorisiert.')); ?></p>
                        </div>
                        <div class="operations-workload">
                            <strong><?php echo esc_html($operations_open_work_count); ?></strong>
                            <span><?php echo esc_html(alpenia_travel_t('offene Arbeitspunkte')); ?></span>
                        </div>
                    </div>

                    <div class="operations-status-grid">
                        <article>
                            <span><?php echo esc_html(alpenia_travel_t('Neu')); ?></span>
                            <strong><?php echo esc_html($participant_status_counts['neu']); ?></strong>
                        </article>
                        <article>
                            <span><?php echo esc_html(alpenia_travel_t('In Prüfung')); ?></span>
                            <strong><?php echo esc_html($participant_status_counts['in_pruefung']); ?></strong>
                        </article>
                        <article>
                            <span><?php echo esc_html(alpenia_travel_t('Vollständig')); ?></span>
                            <strong><?php echo esc_html($participant_status_counts['vollstaendig']); ?></strong>
                        </article>
                        <article>
                            <span><?php echo esc_html(alpenia_travel_t('Fehlende Unterlagen')); ?></span>
                            <strong><?php echo esc_html($missing_docs_count); ?></strong>
                        </article>
                    </div>

                    <div class="operations-detail-grid">
                        <div class="operations-card">
                            <h3><?php echo esc_html(alpenia_travel_t('Dokumenten-Engpässe')); ?></h3>
                            <?php if (!empty($operation_doc_bottlenecks)) : ?>
                                <div class="operations-bottleneck-list">
                                    <?php foreach ($operation_doc_bottlenecks as $doc_label => $doc_count) : ?>
                                        <div class="operations-bottleneck-row">
                                            <span><?php echo esc_html(alpenia_travel_t($doc_label)); ?></span>
                                            <strong><?php echo esc_html($doc_count); ?></strong>
                                        </div>
                                    <?php endforeach; ?>
                                </div>
                            <?php else : ?>
                                <p><?php echo esc_html(alpenia_travel_t('Aktuell keine Dokumenten-Engpässe.')); ?></p>
                            <?php endif; ?>
                        </div>

                        <div class="operations-card">
                            <h3><?php echo esc_html(alpenia_travel_t('Backoffice-Aufgaben pro Reise')); ?></h3>
                            <?php if (!empty($operation_trip_summaries)) : ?>
                                <div class="operations-trip-list">
                                    <?php foreach ($operation_trip_summaries as $stats) : ?>
                                        <article class="operations-trip-row">
                                            <a href="<?php echo esc_url(alpenia_dashboard_link(['view_trip' => $stats['trip_id']])); ?>"><?php echo esc_html($stats['title']); ?></a>
                                            <span><?php echo esc_html(sprintf(alpenia_travel_t('%1$d Unterlagen · %2$d Zahlungen · %3$d in Prüfung · %4$d neu'), $stats['missing_docs'], $stats['open_payments'], $stats['in_review'], $stats['new_participants'])); ?></span>
                                        </article>
                                    <?php endforeach; ?>
                                </div>
                            <?php else : ?>
                                <p><?php echo esc_html(alpenia_travel_t('Aktuell keine offenen Backoffice-Aufgaben pro Reise.')); ?></p>
                            <?php endif; ?>
                        </div>

                        <div class="operations-card operations-card--wide">
                            <h3><?php echo esc_html(alpenia_travel_t('Heute abarbeiten')); ?></h3>
                            <?php if (!empty($operations_task_items)) : ?>
                                <div class="operations-task-list">
                                    <?php foreach ($operations_task_items as $task) : ?>
                                        <article class="operations-task-item">
                                            <div>
                                                <span><?php echo esc_html($task['trip_title']); ?></span>
                                                <a href="<?php echo esc_url(alpenia_dashboard_link(['edit_participant' => $task['participant_id']])); ?>"><?php echo esc_html($task['participant_name']); ?></a>
                                            </div>
                                            <p><?php echo esc_html(implode(' · ', $task['reasons'])); ?></p>
                                            <div class="operations-task-actions">
                                                <a class="table-btn" href="<?php echo esc_url(alpenia_dashboard_link(['view_trip' => $task['trip_id']])); ?>"><?php echo esc_html(alpenia_travel_t('Reise öffnen')); ?></a>
                                                <a class="table-btn" href="<?php echo esc_url(alpenia_dashboard_link(['edit_participant' => $task['participant_id']])); ?>"><?php echo esc_html(alpenia_travel_t('Teilnehmer öffnen')); ?></a>
                                            </div>
                                        </article>
                                    <?php endforeach; ?>
                                </div>
                            <?php else : ?>
                                <p><?php echo esc_html(alpenia_travel_t('Heute sind keine priorisierten Aufgaben offen.')); ?></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </section>

                <section class="analysis-overview" aria-label="<?php echo esc_attr(alpenia_travel_t('Wachstumsanalyse')); ?>">
                    <div class="analysis-overview__header">
                        <div>
                            <span class="overview-card__eyebrow"><?php echo esc_html(alpenia_travel_t('Analyse')); ?></span>
                            <h2><?php echo esc_html(alpenia_travel_t('Wachstumsanalyse')); ?></h2>
                            <p><?php echo esc_html(alpenia_travel_t('Monatsvergleich, Reisearten, Märkte und Team-Leistung als Grundlage für Skalierung.')); ?></p>
                        </div>
                    </div>

                    <div class="analysis-kpi-grid">
                        <article>
                            <span><?php echo esc_html(alpenia_travel_t('Anmeldungen aktueller Monat')); ?></span>
                            <strong><?php echo esc_html($growth_current_month_participants); ?></strong>
                            <small><?php echo esc_html(sprintf(alpenia_travel_t('%+d%% zum Vormonat'), $growth_participant_delta)); ?></small>
                        </article>
                        <article>
                            <span><?php echo esc_html(alpenia_travel_t('Umsatz aktueller Monat')); ?></span>
                            <strong>€ <?php echo esc_html(number_format($growth_current_month_revenue, 2, ',', '.')); ?></strong>
                            <small><?php echo esc_html(sprintf(alpenia_travel_t('%+d%% zum Vormonat'), $growth_revenue_delta)); ?></small>
                        </article>
                        <article>
                            <span><?php echo esc_html(alpenia_travel_t('Stärkste Reiseart')); ?></span>
                            <strong><?php echo esc_html(alpenia_travel_translate_label($growth_top_type_label)); ?></strong>
                            <small><?php echo esc_html(alpenia_travel_t('Nach Teilnehmerzahl')); ?></small>
                        </article>
                        <article>
                            <span><?php echo esc_html(alpenia_travel_t('Stärkster Markt')); ?></span>
                            <strong><?php echo esc_html($growth_top_location_label); ?></strong>
                            <small><?php echo esc_html(alpenia_travel_t('Land / Stadt')); ?></small>
                        </article>
                    </div>

                    <div class="analysis-detail-grid">
                        <div class="analysis-card">
                            <h3><?php echo esc_html(alpenia_travel_t('Reisearten-Auswertung')); ?></h3>
                            <?php if (!empty($growth_type_breakdown)) : ?>
                                <div class="analysis-rank-list">
                                    <?php foreach ($growth_type_breakdown as $item) : ?>
                                        <article>
                                            <div>
                                                <strong><?php echo esc_html(alpenia_travel_translate_label($item['label'])); ?></strong>
                                                <span><?php echo esc_html(sprintf(alpenia_travel_t('%1$d Teilnehmer · %2$d Reisen'), $item['participants'], $item['trips'])); ?></span>
                                            </div>
                                            <em>€ <?php echo esc_html(number_format($item['revenue'], 2, ',', '.')); ?></em>
                                        </article>
                                    <?php endforeach; ?>
                                </div>
                            <?php else : ?>
                                <p><?php echo esc_html(alpenia_travel_t('Noch keine Reisearten-Daten vorhanden.')); ?></p>
                            <?php endif; ?>
                        </div>

                        <div class="analysis-card">
                            <h3><?php echo esc_html(alpenia_travel_t('Länder- und Städte-Auswertung')); ?></h3>
                            <?php if (!empty($growth_location_breakdown)) : ?>
                                <div class="analysis-rank-list">
                                    <?php foreach ($growth_location_breakdown as $item) : ?>
                                        <article>
                                            <div>
                                                <strong><?php echo esc_html($item['label']); ?></strong>
                                                <span><?php echo esc_html(sprintf(alpenia_travel_t('%d Teilnehmer'), $item['participants'])); ?></span>
                                            </div>
                                            <em>€ <?php echo esc_html(number_format($item['revenue'], 2, ',', '.')); ?></em>
                                        </article>
                                    <?php endforeach; ?>
                            ...
                            <?php else : ?>
                                <p><?php echo esc_html(alpenia_travel_t('Noch keine Markt-Daten vorhanden.')); ?></p>
                            <?php endif; ?>
                        </div>

                        <div class="analysis-card">
                            <h3><?php echo esc_html(alpenia_travel_t('Reiseleiter-Auswertung')); ?></h3>
                            <?php if (!empty($growth_guide_breakdown)) : ?>
                                <div class="analysis-rank-list">
                                    <?php foreach ($growth_guide_breakdown as $item) : ?>
                                        <article>
                                            <div>
                                                <strong><?php echo esc_html($item['label']); ?></strong>
                                                <span><?php echo esc_html(sprintf(alpenia_travel_t('%1$d Teilnehmer · %2$d Reisen'), $item['participants'], $item['trips'])); ?></span>
                            ...
                            <?php else : ?>
                                <p><?php echo esc_html(alpenia_travel_t('Noch keine Reiseleiter-Daten vorhanden.')); ?></p>
                            <?php endif; ?>
                        </div>

                        <div class="analysis-card analysis-card--focus">
                            <h3><?php echo esc_html(alpenia_travel_t('Marketing-Fokus')); ?></h3>
                            <?php if (!empty($growth_focus_trips)) : ?>
                                <div class="analysis-focus-list">
                                    <?php foreach ($growth_focus_trips as $stats) : ?>
                                        <article>
                                            <div>
                                                <a href="<?php echo esc_url(alpenia_dashboard_link(['view_trip' => $stats['trip_id']])); ?>"><?php echo esc_html($stats['title']); ?></a>
                                                <span><?php echo esc_html(sprintf(alpenia_travel_t('%1$d%% Auslastung · %2$d/%3$d Plätze'), $stats['occupancy'], $stats['participants'], $stats['capacity'])); ?></span>
                                            </div>
                                            <?php if (alpenia_user_can_edit_trip($stats['trip_id'])) : ?>
                                                <a class="table-btn" href="<?php echo esc_url(alpenia_dashboard_link(['edit_trip' => $stats['trip_id']])); ?>"><?php echo esc_html(alpenia_travel_t('Reise bearbeiten')); ?></a>
                                            <?php else : ?>
                                                <a class="table-btn" href="<?php echo esc_url(alpenia_dashboard_link(['view_trip' => $stats['trip_id']])); ?>"><?php echo esc_html(alpenia_travel_t('Reise öffnen')); ?></a>
                                            <?php endif; ?>
                                        </article>
                                    <?php endforeach; ?>
                                </div>
                            <?php else : ?>
                                <p><?php echo esc_html(alpenia_travel_t('Aktuell keine offenen Reisen mit niedriger Auslastung.')); ?></p>
                            <?php endif; ?>
                        </div>
                    </div>
                </section>
